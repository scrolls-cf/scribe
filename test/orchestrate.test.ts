...
import { OrchestrationGateError } from "../src/orchestration-gates.ts";
...
${"substantive spec content for implement gate ".repeat(4)}
...
describe("applyShip", () => {
	it("marks spec shipped and plan done", () => {
		const spec = baseSpec({
			review_gate: "passed",
			status: "in_progress",
			body: `${"# Goal\n\n".repeat(12)}## Implementation status\n\n| **Status** | Pending |\n`,
		});
	...
	it("rejects ship when acceptance criteria are open", () => {
		const spec = baseSpec({
			review_gate: "passed",
			status: "in_progress",
			body: `## Goal\n\n${"x".repeat(100)}\n\n## Acceptance criteria\n\n- [ ] not done\n`,
...
describe("applyPlanCreated", () => {
	it("forces plan review for thin boilerplate plans", () => {
		const spec = baseSpec({
			review_gate: "passed",
			status: "ready",
			plan_review: "n/a",
...
Generated by \`scripts/pipeline/spec-to-plan.mjs\`
...
`,
		...
		assert.equal(nextSpec.plan_review, "required");
	});
	...
	it("links plan ready when plan_review n/a and plan is substantive", () => {
		const spec = baseSpec({
			review_gate: "passed",
			status: "ready",
			plan_review: "n/a",
		...
		const plan = basePlan({
			status: "blocked",
			body: `## Phase 1 — Deploy
- [ ] npm run cf:deploy ged-x402
- [ ] npm run x402:smoke
- [ ] Record production origin in spec
`,
		});
...
describe("applyPhaseDone", () => {
	it("marks phase done and activates next with gate evidence", () => {
		...
		const { plan: nextPlan } = applyPhaseDone(spec, plan, {
			phase_id: "p1",
			gate_command: "npm run test:pipeline-smoke",
			stdout_hash: "deadbeef",
		});
		assert.equal(nextPlan.phases.find((p) => p.id === "p1")?.status, "done");
		assert.equal(nextPlan.phases.find((p) => p.id === "p1")?.gate_command, "npm run test:pipeline-smoke");
		assert.equal(nextPlan.phases.find((p) => p.id === "p2")?.status, "active");
		assert.equal(nextPlan.status, "in_progress");
	});

	it("blocks implement phase done without gate evidence", () => {
		const spec = baseSpec({ review_gate: "passed", status: "in_progress" });
		const plan = basePlan({
			status: "in_progress",
			phases: [
				{ id: "p1", index: 1, title: "Deploy", status: "active", body: "", lock: null },
			],
		});
		assert.throws(
...
describe("applyImplementStart", () => {
	it("promotes ready plan to in_progress", () => {
		const spec = baseSpec({
			...
			status: "ready",
			plan_review: "passed",
			body: `## Goal\n\n${"implementation scope ".repeat(8)}\n\n## Implementation status\n\n| **Status** | Pending |\n`,
		});
		const plan = basePlan({
			status: "ready",
			body: `${"# Plan\n\n".repeat(5)}- [ ] npm run verify\n- [ ] npm run deploy\n- [ ] npm run smoke\n`,
		});
		const { plan: nextPlan } = applyImplementStart(spec, plan);
		assert.equal(nextPlan.status, "in_progress");
	});

	it("blocks implement when spec body is empty", () => {
		const spec = baseSpec({ review_gate: "passed", status: "ready" });
		const plan = basePlan({ status: "ready" });
		assert.throws(
			() => applyImplementStart(spec, plan),
			(err: unknown) =>
				err instanceof OrchestrationGateError && err.code === "spec_body_empty",
		);
	});
